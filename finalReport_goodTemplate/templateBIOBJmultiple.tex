...
\keywords{Evolutionary algorithm, Benchmarking, Black-box optimization,
  Bi-objective optimization, Decision Vector, Pareto-Set approximation}
...
consider several solutions at the same time. In single-objective optimizatin no mating
...
Our binary quality indicator is defined as the minimum distance by which a
Pareto set approximation needs to be translated in each dimension in the objective space so that another approximation (image of a decision space vector) is weakly dominated. The weak domination is defined as follows: decision vector $x^1$ weakly dominates $x^2$, written $x^1 \succeq x^2$, if $x^1$ dominates $x^2$ or the corresponding objective vectors are equal \cite{IBEA_base}. The mathematical translation of this definition consists in the following equation:
...
Mating selection aims at picking promising solutions for variation and usually
is performed in a randomized fashion. It performs a binary tournament selection with replacement on $P$ in order to fill a temporary mating pool (the variable $P'$ in our implementation). The binary tournament consists in keeping the candidate with the best fitness value from the random pick.

\subsection{Details regarding mutation}

The mutation operator modifies individuals by changing small parts in the associated vectors according to a given mutation rate. The mutation rate determines how much of the considered population we will use to generate new decision space vectors. This process reminds how evolutionary algorithms seem related to biological evolution, mimicking natural hereditary relationships. For mutation, we used a polynomial mutation operator.
...
Furthermore, the complexity in memory is $O(P^2)$\\
...
Step 2, which is the most critical one, is perfectly parallelizable. Storing
$-I_\epsilon(\{x\},\{y\})$ in a hashmap of hashmap do the job perfectly, while
a unique hashmap would have caused problems. Doing this, other steps are also
greatly parallelizable.\\
However, implementing multi-threading to be more efficient would have been
tedious and prone to errors. Considering that, we instead chose to stay with an easier
implementation and run several instances of benchmarking together to
better use computing units of our machines.
